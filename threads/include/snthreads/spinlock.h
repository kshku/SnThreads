...
#define sn_spinlock_lock(lock)                                                  \
    while (sn_atomic_flag_test_and_set_explicit(lock, SN_MEMORY_ORDER_ACQUIRE)) \
 ...
 */
#define sn_spinlock_unlock(lock) sn_atomic_flag_clear_explicit(lock, SN_MEMORY_ORDER_RELEASE)
